...
export const WeaponCard = ({data}: ICard) => {
  return (
    ...
    href={`/simulacra/${data.id}`} style={{backgroundColor: Colors['WeaponCard']}}>
      <div className='absolute top-0 left-7 bg-no-repeat rounded-lg w-full h-full scale-125 bg-cover bg-center ease-in-out duration-[250ms] delay-0 group-hover:scale-150' style={{backgroundImage: `url(https://api.toweroffantasy.info${data.assets.icon})`}}/>
